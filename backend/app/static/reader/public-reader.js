...
(() => {
  ...
  function draw() {
    ...
    if (connection.managed) {
      section.innerHTML = `<h2>模型连接</h2><p>评审体验期间已由服务端接入 DeepSeek，无需填写 API Key，可直接运行研究功能。</p>
        <p id="publicConnectionStatus" role="status">已连接 ${e(connection.model)}（非思考模式）</p>
        <p class="settings-count-note">密钥仅保存在服务端部署环境，不会下发到浏览器。只有主动运行研究时才会向 DeepSeek 发送当前受控事实包。</p>
        <details><summary>清空本次体验</summary><p>删除本会话的项目、草稿和研究记录。请先导出需要的内容。</p><button type="button" class="ghost-btn" data-demo-reset>清空我的体验数据</button></details>`;
      section.querySelector('[data-demo-reset]').onclick = resetDemo;
      return;
    }
    section.innerHTML = `<h2>模型连接</h2><p>使用自己的 API 进行研究，连接仅在本次体验保留，最长一小时。</p>
      ...
      <label>模型名称<input name="model" required maxlength="150" value="${e(connection.model || 'deepseek-flash')}"></label>
      ...
      <p class="settings-count-note">DeepSeek 快捷配置使用 Flash 非思考模式，适用于本例短研究任务。未连接时不调用公用密钥。</p></form>
    ...
    section.querySelector('[data-deepseek]').onclick = () => {
      ...
      form.elements.model.value='deepseek-flash'; form.elements.consent.checked=false;
    };
    section.querySelector('[data-demo-reset]').onclick = resetDemo;
  }
  ...
  window.ReaderAuth.init = async () => {
    ...
    banner.innerHTML=`公开体验 · 刚果（金）精选历史样本 <a href="#/settings">${connection.managed ? 'DeepSeek 体验已连接' : '连接自己的 API'}</a>`;
